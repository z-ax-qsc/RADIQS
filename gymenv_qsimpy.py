...
class QSimPyEnv(gym.Env):
    ...
    Args:
        ...
        algo (str): algorithm used to run
    ...
    def __init__(
        ...
        self.n_qnodes = config.get("qnodes", 5)  # number of qnodes
        self.qtasks = []
        self.qnodes = []
        self.mode = mode
        ...
        max_clops = 1000000  # Max computational load
        ...
        self.algo = algo or 'Unknown';

        # networked parameters
        self.task_per_set = config.get("task_per_set", 10) 
    ...
    def setup_quantum_resources(self, qnodes = None, qnode_names=None, edge_prob = 0.3):
        # Create a list of 10 IBM QNodes
        qnode_ids = range(self.n_qnodes if qnodes is None else qnodes)
        # qnode_names = [
        #     "washington",
        #     "kolkata",
        #     "hanoi",
        ...
        #     "lagos",
        # ]
        if qnode_names is None:
            qnode_names = self.rng.choice(list(IBMQNode.ibmq_systems.keys()), len(qnode_ids), replace=True);
        else:
            qnode_names = self.rng.choice(qnode_names, len(qnode_ids), replace=True);
        # print(qnode_names)

        #Ids are in order from 0-n
        self.qnodes = [
            IBMQNode.create_ibmq_node(self.qsp_env, qid, qname)
            for qid, qname in zip(qnode_ids, qnode_names)
        ]
        self.node_graph = nx.fast_gnp_random_graph(len(self.qnodes), p = edge_prob, seed = self.rng); 
        # self.node_graph = nx.complete_graph(len(self.qnodes)); 
    ...
    def generate_qtasks(self):
        ...
        self.n_qtasks = n_qtasks;

        # QTask IDs
        qtask_ids = list(
            f"{self.round:04d}" + f"{id:02d}" for id in range(n_qtasks)
        )
        ...
        if self.policy == NETWORK_POLICY:
            iter = n_qtasks//self.task_per_set
            for i in range(iter):
                id = i*self.task_per_set;
                qtask_arrival[id:id+self.task_per_set] = qtask_arrival[id];
            if iter*self.task_per_set < len(qtask_arrival):
                qtask_arrival[iter*self.task_per_set: ] = qtask_arrival[iter*self.task_per_set]
        ...
        # Set initial task to consider
        if self.policy == NETWORK_POLICY:
            indx = min(len(self.qtasks), self.task_per_set);
            self.current_qtask = self.qtasks[:indx]
            # del self.qtasks[:indx];
        else:
    ...
    def submit_task_to_qnode(self, qtask, qnode_id=None, task_graph = None, map = None):
        reward = 0
        if qnode_id is None:
            qnode_id = self.round_robin_index % self.n_qnodes
            self.round_robin_index += 1
        qtask, waiting_time, execution_time = self.broker.preprocess_qtask(
            qtask, self.qnodes[qnode_id], task_graph , self.qnodes, self.current_qtask, map
        )
        if qtask.status == TaskStatus.ERROR:
            ...
            # This will not be called for network policy, as checking qubit condition before assignment
            ...
            while (
                ...
                index += 1
            #rescheduling condtion
            self.qtasks.insert(index, qtask)
            return -0.1, qtask.rescheduling_count #note: -10 coming from here 
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if self.policy == NETWORK_POLICY:
            self.current_obs = (self.current_qtask, self.qnodes)
        else:
            self.current_obs = self._get_obs().astype(np.float32)
        info = {}
        return self.current_obs, info
    
    def collect_results(self, decision_time):
        Log.print_simulation_results(self.qnodes)
        Log.export_simulation_results(decision_time, self.qnodes, self.params, self.unfulfilled_tasks, output_file=self.policy)
    ...
    def step_network(self, qnodes, task_graph):
        # Submit the currentn set of qtasks to the selected qnodes
        # Intermediately reward is the inverse of completion time
        # Sample Objective: Minimize the total completion time of all qtasks
        if qnodes == None or len(qnodes) == 0:
            print("No nodes found for mapping tasks: ", [t.id for t in self.current_qtask]);
            reward = 0;
            scheduled_qtask = None;
            self.unfulfilled_tasks.extend([t for t in self.current_qtask]) 
        else:
            reward = 0;
            for i in range(len(qnodes)):
                time_reward, _ = self.submit_task_to_qnode(
                    self.current_qtask[i], qnodes[i], task_graph, {k:v for k,v in  zip([t.id for t in self.current_qtask], qnodes)}
                )
                reward += 1/time_reward
            # print("Scheduled: ", [t.id for t in self.current_qtask]);
            scheduled_qtask = self.current_qtask

        # Get the next set of observation
        # Check if there are more qtasks, if yes, get the next set of qtasks, otherwise set terminated to True
        if len(self.qtasks) > 0:
            indx = min(len(self.qtasks), self.task_per_set);
            del self.qtasks[:indx];
            terminated = False;
    ...
    def close(self, decision_time):
        # If the evaluation is set, run the environment and export the results
        if self.evaluation:
            Log.log = True
            self.qsp_env.run() #Check: No use
            # Log.print_simulation_results(self.qnodes)
            self.collect_results(decision_time)
        pass
