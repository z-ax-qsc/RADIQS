...
from qsimpy.utils.Metrics import error, neighborCommunicationCost
...
class Broker:
    ...
    def preprocess_qtask(self, qtask, qnode, task_graph, nodes, tasks, mappings):
        ...
        if not satisfied:
            ...
            # For networked case
            if task_graph is not None and mappings is not None:
                qtask.communication_cost = neighborCommunicationCost(task_graph, qnode, qtask, mappings, nodes, tasks);
            ...
            qtask.fidelity = 1 - error(qnode, qtask);
