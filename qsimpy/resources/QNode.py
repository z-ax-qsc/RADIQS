...
class QNode(simpy.Resource):
    ...
    def task_run(self, task):
        return (task.get_circuit_layers() / self.clops) * task.shots;
    ...
    def process_task(self, task):
        ...
        execution_time = self.task_run(task)
    ...
    def simulate_task(self, task):
        ...
        # )
        task.transpiled_circuit_layers = transpiled_cl;
        task.transpiled_gate_counts = task.qtask_data[self.qnode_model]["gates"]
        task.cal_transpiled_2q_gates();
        # self.completed_tasks.append(task)
        # task.set_status(TaskStatus.DONE)
    ...
    def emulate_task(self, task):
        ...
        if qasm_path:
            ...
            task.execution_time=self.task_run(task)
    ...
    def get_estimated_execution_time(self, task, mode="simulate"):
        ...
        #CARE Formula should be same as that in metrics file
        if mode=="simulate":
