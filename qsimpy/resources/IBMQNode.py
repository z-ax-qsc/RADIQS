...
from qsimpy.utils.Metrics import ErrorKeys, TimeKeys
import enum
...
def map_keys(values, keys: enum.Enum):
    # values should be in order of keys;
    return {k.value:v for k, v in zip(keys, values)};
...
ibmq_errors = {
    "brisbane": map_keys([220.53e-6, 128.92e-6, 2.393e-2, 2.517e-4, 7.042e-3, 3.29e-3, 1.59e-2], ErrorKeys),
    "torino": map_keys([181.41e-6, 138.75e-6, 2.991e-2, 3.296e-4, 2.68e-3, 1.11e-3, 1.10e-2], ErrorKeys),
    "marrakesh": map_keys([188.11e-6, 111.97e-6, 1.074e-2, 3.047e-4, 2.451e-3, 1.06e-3, 3.47e-3], ErrorKeys),
...
ibmq_runtimes = {
    "brisbane": map_keys([60e-9, 660e-9, 1600e-9], TimeKeys),
    "torino": map_keys([32e-9,  68e-9, 1560e-9], TimeKeys),
    "marrakesh": map_keys([36e-9, 68e-9, 2584e-9], TimeKeys),
}


# IBM Quantum QV and CLOPS
ibmq_systems = {
    "brisbane": {"qubits": 127, "clops": 180000},
    "torino": {"qubits": 133, "clops": 220000},
    "marrakesh": {"qubits": 156, "clops": 200000},
    ...
    #Check 5 qubit not supported. dependent on Qtast data 
}
...
def create_ibmq_node(env, id: int, backend_name: str):
    ...
    ibmq_node.quantum_volume = node_metrics.get("qv")
    ...
    ibmq_node.error = ibmq_errors.get(backend_name)
    ibmq_node.runtimes = ibmq_runtimes.get(backend_name)
    return ibmq_node
