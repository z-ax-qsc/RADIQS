...
import qiskit
from qsimpy.utils.Metrics import GATES2Q,MEASURE
...
class QTask:
    ...
    def __init__(
        ...
        self.gates_2q = self.cal_2q_gates(self.gate_counts);
        ...
        self.transpiled_gates_2q = 0
        self.fidelity = 0
        self.communication_cost = 0

    @staticmethod
    def cal_2q_gates(gates):
        if gates is not None:
            return sum([g[1] for g in gates.items() if 'c' in g[0]]);
    ...
    def generate_att_dict(self):
        return {"qubit_number": self.qubit_number};

    def get_node_circuit_layers(self, model):
        return self.qtask_data[model]["depth"]
    
    def cal_transpiled_2q_gates(self):
        self.transpiled_gates_2q = sum([g[1] for g in self.transpiled_gate_counts if g[0] in GATES2Q]); 

    def get_node_circuit_2qgates(self, model):
        return sum([g[1] for g in self.qtask_data[model]["gates"] if g[0] in GATES2Q]); 

    def get_node_circuit_measure(self, model):
        return sum([g[1] for g in self.qtask_data[model]["gates"] if g[0] == MEASURE]); 